...
import { stat, readFile, access, realpath, open, unlink } from 'node:fs/promises';
...
function extractConfigFiles(args) {
    const configFiles = [];
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (argument.startsWith('--config-file=')) {
            ...
            if (!value)
                throw new Error('--config-file requires a path');
        ...
        if (!foundValue)
            throw new Error('--config-file requires at least one path');
    }
    return configFiles;
}
async function discoverProviders(configFiles, cwd = process.cwd()) {
    ...
    for (const configFile of configFiles) {
        ...
        if (fileStats.size > MAX_CONFIG_BYTES) {
            throw new Error(`Config file exceeds the 5 MiB safety limit: ${configFile}`);
        }
        let contents;
        try {
        ...
        if (document.errors.length > 0) {
            throw new Error(`Malformed YAML in ${configFile}: ${document.errors[0].message}`);
        }
...
async function withInstallLock(environment, operation) {
    ...
    while (true) {
        try {
            ...
            if (Date.now() >= deadline) {
                throw new Error('Timed out waiting for another Anvil dependency installation', { cause: error });
            }
            await delay(100);
        }
    }
}
async function runShim(originalArgs) {
    ...
    const relativeToShim = relative(await realpath(shimDirectory), resolvedRealAnvil);
    if (relativeToShim === '' ||
...
try {
    ...
    console.error(`setup-anvil: ${error instanceof Error ? error.message : String(error)}`);
    process.exitCode = 1;
}
